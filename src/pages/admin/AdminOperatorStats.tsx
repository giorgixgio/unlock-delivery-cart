...
import { Activity, AlertTriangle, HelpCircle, Loader2, RefreshCw, Users } from "lucide-react";
...
export default function AdminOperatorStats(){
  ...
  const overview=useMemo(()=>calculateOverview(visibleOrders,visibleCalls,openOrders),[visibleOrders,visibleCalls,openOrders,firstTouches,callEvents]);
  ...
  const operatorRows=useMemo(()=>operators.map(name=>{
    ...
    const firstForOperator:firstTouches extends never ? never[] : Session[]=[]; const firstSeen=new Map<string,Session>();for(const s of firstTouches){const p=firstSeen.get(s.order_id);if(!p||s.session_started_at<p.session_started_at)firstSeen.set(s.order_id,s);}for(const s of firstSeen.values())if(s.operator===name)firstForOperator.push(s);
