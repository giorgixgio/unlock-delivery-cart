...
const StickyCartHUD = () => {
  ...
  return (
    <div className={`fixed bottom-0 left-0 right-0 z-50 bg-card border-t-2 shadow-lg transition-all duration-300 ${isUnlocked ? "border-success glow-unlock" : "border-primary"} ${justUnlocked ? "animate-glow-pulse" : ""}`}>
        ...
        <div className="flex items-center gap-2">
          <div className="flex gap-1.5 flex-shrink-0 overflow-x-auto">
            {items.slice(0, 5).map((item) => (
              <div key={item.product.id} className="relative w-9 h-9 rounded-md overflow-hidden border border-border flex-shrink-0 animate-scale-in">
                <img src={item.product.image} alt={item.product.title} className="w-full h-full object-cover" />
                {item.quantity > 1 && (
                  <span className="absolute -top-1 -right-1 bg-primary text-primary-foreground text-[9px] font-bold w-3.5 h-3.5 rounded-full flex items-center justify-center">{item.quantity}</span>
                )}
              </div>
            ))}
            {items.length > 5 && (
              <div className="w-9 h-9 rounded-md bg-muted flex items-center justify-center text-[10px] font-bold text-muted-foreground border border-border">+{items.length - 5}</div>
            )}
          </div>
          <div className="ml-auto"><SaleTotalDisplay size="sm" /></div>
        </div>
        <DeliveryMissionBar mini />
        {isUnlocked ? (
          <div className="flex items-center justify-center gap-1.5 py-0.5 animate-success-reveal">
            <CheckCircle className="w-3.5 h-3.5 text-success" />
            <span className="text-xs font-bold text-success">მიტანა უფასო!</span>
          </div>
        ) : (
          <DeliveryInfoMini />
        )}
